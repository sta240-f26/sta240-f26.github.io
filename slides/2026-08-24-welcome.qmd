...
library(tidyverse)
gs4_deauth()
d <- read_sheet("https://docs.google.com/spreadsheets/d/1mzTXIrLUMITbqh0shKAtP1K0Ovy_kzd0D6uGKad1V5M/edit?usp=sharing") |>
  rename(birthday = `What is your birthday?`,
         ...
         price = `If you go to The Regulator Bookshop on 9th Street, what is the after-tax price (in USD) of this newly released hardcover book?`,
...
::: {.column width="50%"}
{{< qrcode https://docs.google.com/forms/d/e/1FAIpQLSeaS7A98OGoPQKvsOKHk4IQjZyYMfirvb7a-SgfGEcIUMPsfg/viewform?usp=dialog width=300 height=300 >}}
:::
:::::
...
|-----------------------|-------------|------------|---------------|
| ![](images/ch.jpg){.c} | Huang, Claire   | TA         | Th 10 am - 12 pm           |
| ![](images/dk.jpg){.c} | Kakkad, Dwija | Lab TA    | Wed 10 am - 12 pm |
| ![](images/ru.jpg){.c} | Urena, Ricardo   | TA         | Sun 7:30 pm - 9:30 pm |
| ![](images/z.jpg){.c} | Zito, John  | Instructor | F 1:00 pm - 3:30 pm |
: {tbl-colwidths="[20,25,20,35]"}
...
ggplot(d, aes(x = bday_prob)) + 
  geom_histogram() + 
  labs(
    title = "How many people must attend for a \n50% chance of a birthday match?",
    x = "Number of people"
  ) + 
...
ggplot(d, aes(x = potato)) + 
  ...
  geom_vline(xintercept = 17, col = "red", size = 2)
```
...
ggplot(d, aes(x = price)) + 
  geom_histogram() + 
  labs(
    title = "How much does this new, hardcover book cost\nat The Regulator Bookshop on 9th Street?",
    x = "After-tax price ($)"
  ) + 
  geom_vline(xintercept = 31.18, col = "red", size = 2) + 
  theme_minimal(base_size = 24)
```
...
## Example: the mathematics of insurance {.small}
...
1. [***Data Scientist***]{style="color:red;"}
1. Speech-Language Pathologist
1. Pilot
1. [***Actuary***]{style="color:red;"}
:::

::: {.column width="50%"}

[![](images/usnews.png){fig-align="center" width="50%"}](https://careers.usnews.com/best-jobs/rankings/the-100-best-jobs)
...
::::: {.columns .v-center-container}
::: {.column width="30%"}
{{< qrcode https://sta240-f26.github.io/ width=300 height=300 >}}
:::

::: {.column width="70%"}
<https://sta240-f26.github.io/>
:::
:::::
...
The final letter grade will be based on the usual thresholds, which will not change and will be applied exactly. So no curve, no rounding, no extra credit.
:::
...
Lead by Dwija in Perkins LINK 060 (Classroom 1):

-   Thursday 1:25 PM - 2:40 PM;
-   Thursday 3:05 PM - 4:20 PM.

Guided R activities about the interplay between math and *computer simulation*. Completed in randomly-assigned groups and due at the end of the period. No late work accepted, and you must be in attendance to earn credit.

## Problem Sets (20%)

-   Pencil-and-paper math problems and simulation exercises;
...
-   **Midterm 1**: Thursday October 8 *in your lab*;
-   **Midterm 2**: Thursday November 12 *in your lab*;
-   **Final Exam**: Monday December 14 from 2 PM to 5 PM.
...
- **Lab**: required if you want credit for the graded assignments. You can't abandon your group after all. I let our "drop 2" policy automatically handle most attendance hiccups.

. . .

:::callout-warning
## Go on. Cut the umbilical chord.
If you choose to attend lecture, please put your laptop away.
:::
...
**REVIEW**: [Problem Set 0](/problems/pset-0.html) is due 5PM Friday September 4.
...
- Lab on Thursday August 27 is a review session and work period for this assignment;
- The course page has a [math review section](/math/differentiation.html) you can refer to.
:::

## Whence all the math?

Statistics is a mathematical discipline. In its way, it's as mathematical as physics. Why?

::: incremental
- In more applied courses, we teach you all sorts of methods for analyzing data: linear and logistic regression, the bootstrap, hypothesis testing, and so on;
- Academic statisticians zoom out from the applications and ask:
    - do these methods actually *work*?
    - what does it even *mean* for a method to "work"?
- These are *theoretical* questions, and they require serious mathematics to answer.
:::

## Whence all the math?

For those of you considering a statistical science major, STA 240 is the first in a "Theory Trilogy:"

- STA 240 on probability theory;
- STA 332 on classical mathematical statistics;
- STA 402 on Bayesian statistics.

It all builds. 240 lays the technical foundations, and you must master them to succeed in the major.
...
- **Wednesday lecture**: set theory;
- **Thursday lab**: calculus review;
- **First OHs**: Friday 1:00 pm - 3:30 pm with JZ.


Also, please complete the [Getting To Know You Survey](https://docs.google.com/forms/d/e/1FAIpQLSfWu2TkoG8_4vskZzoUng4FbvvFC1aqGWtm2l7IjJ-FGch8fQ/viewform).
