...
You first met the gamma function on [Problem Set 0](/problems/pset-0.html#problem-7), where you showed that it satisfies the following *recursion*:
...
The last line follows because $\Gamma(1)=\int_0^\infty \exp(-x)\,\textrm{d}x=1$ (shown [here](/math/integration.html#improper-integration-we-will-do-this-a-lot), but make sure you can do it yourself). The punchline is that the gamma function is a continuous function that *interpolates* the factorials. That is, it's a smooth curve that "connects the dots," like you see in @fig-gamma.

The gamma function has many funky properties that have kept mathematicians busy for a long time. For instance, $\Gamma(1/2)=\sqrt{\pi}$, which we will show at some point. If you're interested, [there's more where that came from](https://en.wikipedia.org/wiki/Particular_values_of_the_gamma_function).
