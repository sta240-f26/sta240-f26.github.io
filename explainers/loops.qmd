...
In [this explainer](/explainers/lineplot.html), we saw how to use the `curve` function to plot lines. If you want a plot with many lines, you can just call the `curve` function many times with the `add = TRUE` argument to layer things. In that explainer, we just hard-coded the multiple calls to `curve`. A for loop can clean that up:
...
That code plots the function $ax^2$ for every value of $a=1,\,2,\,3,\,...,\,25$. Instead of writing 25 calls to the curve function, I consolidate with a loop.
