...
Notice that we made use of the fact that the `dbinom` command is [*vectorized*](/explainers/vectorization.html) in its first argument; if you give it a vector with multiple values, it returns a vector with the probabilities for each one.
